...
func (cfg *config) applierSnap(i int, applyCh chan ApplyMsg) {
	...
	for m := range applyCh {
		...
		if m.SnapshotValid {
			cfg.mu.Lock()
			Debug(dTest, "S%d apply snapshotIndex:%d\n", rf.me, m.SnapshotIndex)
			err_msg = cfg.ingestSnap(i, m.Snapshot, m.SnapshotIndex)
			cfg.mu.Unlock()
		} else if m.CommandValid {
			Debug(dTest, "S%d apply commandIndex:%d expect:%d\n", rf.me, m.CommandIndex, cfg.lastApplied[i]+1)
...
func (cfg *config) end() {
	...
	if cfg.t.Failed() == false {
		...
		fmt.Printf("  Passed --")
		fmt.Printf(" real time:%4.1f \t number of Raft peers:%d \t number of RPC sends:%4d \t number of bytes:%7d \t number of Raft agreements reported:%4d\n", t, npeers, nrpc, nbytes, ncmds)
	}
}
